...
const drawBarsAround = (arrayStrings) => {
  ...
  for (let index = 0; index < arrayStrings.length; index++) {
      ...
      if (index == 0) {
          paddedString = CENTERED_V_LINE.padEnd(text.length+1, text).padEnd(text.length+2, CENTERED_V_LINE);
      } else {
          paddedString = paddedString.concat(text.padEnd(text.length+1, CENTERED_V_LINE));
      };
  }
  return paddedString;
};
