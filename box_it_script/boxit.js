...
const drawTopBorder = (length) => {
  ...
  return TOP_LEFT_CORNER.padEnd(length+1, drawLine(length)).padEnd(length+2, TOP_RIGHT_CORNER);
};

const drawBottomBorder = (length) => {
  if (length < 0) length = 0;
  return BOTTOM_LEFT_CORNER.padEnd(length+1, drawLine(length)).padEnd(length+2, BOTTOM_RIGHT_CORNER);
};

const drawMiddleBorder = (length) => {
  if (length < 0) length = 0;
  return MIDDLE_LEFT_T.padEnd(length+1, drawLine(length)).padEnd(length+2, MIDDLE_RIGHT_T);
};

const drawBarsAround = (text) => {
  return CENTERED_V_LINE.padEnd(text.length+1, text).padEnd(text.length+2, CENTERED_V_LINE);
};
...
const boxIt = (arrayStrings) => {
  ...
  for (let index = 0; index < arrayStrings.length; index++) {
      ...
      paddedString = paddedString.concat(drawBarsAround(text.padEnd(maxLength))).concat('\n');
